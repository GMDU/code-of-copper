...
$loot replace entity 704a8f67-58d9-429d-8535-8eb0957136ca weapon.offhand mine ~$(x) ~$(y) ~$(z) $(id)$(tag) 

data modify entity 704a8f67-58d9-429d-8535-8eb0957136ca HandItems[0] set from storage bot:private temp.place.item
execute store result score $damage bot.interpreter run data get storage bot:private temp.place.tag.Damage
...
$execute unless score $is_broken bot.interpreter matches 1 store result entity @s Items[{Slot:$(slot)b}].tag.Damage int 1 run scoreboard players get $damage bot.interpreter
