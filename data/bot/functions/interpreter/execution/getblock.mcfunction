...
execute on passengers as @s[type=marker,tag=bot.golem.brain] run data modify entity @s data.return_value set value {type: "string", value: ["m","i","n","e","c","r","a","f","t",":","a","i","r"]}
execute on passengers as @s[type=marker,tag=bot.golem.brain] run data modify entity @s data.return_value.value set from storage moxlib:api/string/to_array output
