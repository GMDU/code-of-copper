...
execute unless score $value bot.interpreter matches 0 run data modify storage bot:api/interpreter/function execute.return set from storage bot:interpreter evaluate.stack[-1].args[0].value[0]
execute unless score $value bot.interpreter matches 0 run return -1
