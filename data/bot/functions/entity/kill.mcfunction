execute if data entity @s data.book run summon item ~ ~ ~ {Tags:["bot.golem.drop"],Item:{id:"minecraft:writable_book"}}
data modify entity @e[type=item,limit=1,sort=nearest,tag=bot.golem.drop] Item set from entity @s data.book
execute as @s[tag=bot.golem.display] at @s run loot spawn ~ ~ ~ loot bot:spawn
