...
tellraw @s {"storage":"bot:parser","nbt":"stack[-1].metadata"}
...
execute if data storage bot:parser stack[-1].metadata{status:"closed"} unless data storage bot:parser consumed run function bot:parser/literal/close/init