...
execute unless data storage moxlib:api/string/filter {output:false} run data modify storage bot:parser raise set value '{"text":"[Literal - Alphanumeric]: Expected [A-Za-z0-9_-], received \\"","extra":[{"nbt":"current","storage":"bot:parser"},"\\""]}'